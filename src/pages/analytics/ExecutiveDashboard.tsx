...
import { analyticsService, type DepartmentPerformance, type YoyDataPoint } from "../../services/analyticsService";
...
const TIER_COLORS = ["#10B981", "#3B82F6", "#F59E0B", "#EF4444", "#8B5CF6", "#EC4899"];
...
export default function ExecutiveDashboard() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const [kpiData, trendData, deptPerf, yoy] = await Promise.all([
        analyticsService.getCompanyKPIs(user.companyId, dateRange),
        analyticsService.getIncrementTrends(user.companyId, dateRange),
        analyticsService.getDepartmentPerformance(user.companyId),
        analyticsService.getYoyData(user.companyId),
      ]);
      setKpis(kpiData);
      setTrends(trendData);
      setDeptData(deptPerf);
      setYoyData(yoy);
    } catch (error) {
      console.error(error);
    } finally {
  ...
  const sortedDepts = [...deptData].sort((a, b) => b.averageScore - a.averageScore);
  const topDepts = sortedDepts.slice(0, 3);
  const bottomDepts = sortedDepts.slice(-2).reverse();

  const tierNames = Array.from(
    new Set(yoyData.flatMap(row => Object.keys(row).filter(k => k !== 'year')))
  );
  ...
  return (
    <div className="space-y-6 pb-12">
          ...
          <Link to="/fairness" className="bg-white p-5 rounded-xl border border-slate-200 flex flex-col justify-between hover:border-emerald-300 transition-colors group">
            ...
            <div>
              ...
              <h3 className={`text-2xl font-bold ${kpis.fairnessScore >= 75 ? 'text-emerald-600' : kpis.fairnessScore >= 60 ? 'text-amber-600' : 'text-red-600'}`}>
                {kpis.fairnessScore > 0 ? `${kpis.fairnessScore}/100` : 'N/A'}
              </h3>
            </div>
          </Link>
        </div>
      ...
      <div className="bg-white rounded-xl border border-slate-200 p-6">
        ...
        {trends.length === 0 ? (
          <div className="h-[350px] flex items-center justify-center text-slate-400 text-sm">
            No completed cycles with finalized evaluations yet.
          </div>
        ) : (
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-white rounded-xl border border-slate-200 p-6 min-w-0">
          <h2 className="text-lg font-bold text-slate-900 mb-6">Department Performance</h2>
          {deptData.length === 0 ? (
            <div className="h-[300px] flex items-center justify-center text-slate-400 text-sm">
              No department evaluation data yet.
            </div>
          ) : (
              ...
              <ResponsiveContainer width="100%" height="100%" minWidth={0}>
                <BarChart data={yoyData} margin={{ top: 20, right: 20, left: 0, bottom: 5 }}>
                  ...
                  {tierNames.map((tier, idx) => (
                    <Bar key={tier} dataKey={tier} fill={TIER_COLORS[idx % TIER_COLORS.length]} radius={[2, 2, 0, 0]} />
                  ))}
                </BarChart>
              </ResponsiveContainer>
            </div>
      ...
      <div className="bg-white rounded-xl border border-slate-200 p-6">
        ...
        {trends.length === 0 ? (
          <div className="h-[250px] flex items-center justify-center text-slate-400 text-sm">
            No budget utilization data available yet.
          </div>
        ) : (
          <div className="h-[250px]">
            <ResponsiveContainer width="100%" height="100%" minWidth={0}>
              <AreaChart data={trends} margin={{ top: 10, right: 30, left: 0, bottom: 0 }}>
                ...
                <Area type="monotone" dataKey="budgetUtilization" name="Budget Utilization %" stroke="#10B981" strokeWidth={2} fillOpacity={1} fill="url(#colorBudget)" />
              </AreaChart>
            </ResponsiveContainer>
          </div>
        )}
