...
const SuperAdminDashboard: React.FC = () => {
  ...
  const fetchCompanies = async () => {
    try {
      ...
      // We don't necessarily want to block the whole UI if list fails,
      // but you can log it or show a toast.
    } finally {
  ...
  const handleDeleteCompany = async (id: string) => {
    if (!window.confirm("Are you sure you want to delete this company? Note: This does not delete the admin user.")) {
      return;
    }
    try {
  ...
  return (
    <div className="p-8 font-brand max-w-5xl mx-auto">
      ...
      <div className="bg-white p-8 rounded-2xl shadow-sm border border-gray-100">
        <h2 className="text-2xl font-bold text-merit-navy mb-6">Registered Companies</h2>
          ...
          <div className="overflow-x-auto">
            <table className="min-w-full divide-y divide-gray-200">
              <thead className="bg-gray-50">
                <tr>
                  <th className="px-6 py-3 text-left text-xs font-bold text-merit-slate uppercase tracking-wider">Company</th>
                  <th className="px-6 py-3 text-left text-xs font-bold text-merit-slate uppercase tracking-wider">Contact</th>
                  <th className="px-6 py-3 text-left text-xs font-bold text-merit-slate uppercase tracking-wider">Status</th>
                  <th className="px-6 py-3 text-left text-xs font-bold text-merit-slate uppercase tracking-wider">Registered</th>
                  <th className="px-6 py-3 text-right text-xs font-bold text-merit-slate uppercase tracking-wider">Actions</th>
                </tr>
              </thead>
                ...
                {companies.map((company) => (
                  <tr key={company.id} className="hover:bg-gray-50 transition-colors">
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="text-sm font-bold text-merit-navy">{company.name}</div>
                      <div className="text-xs text-merit-slate mt-1">{company.address || 'No address'}</div>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="text-sm text-gray-900">{company.email || 'No email'}</div>
                      <div className="text-xs text-merit-slate mt-1">{company.mobileNumber || 'No mobile'}</div>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
